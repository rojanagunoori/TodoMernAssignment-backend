...
app.post("/login", (req, res) => {
  ...
  db.get("SELECT * FROM users WHERE email=?", [email], (err, user) => {
    ...
    if (!validPassword)
      return res.status(401).send({ message: "Invalid Paswword" });
...
app.get("/gettasks", authenticateJWT, (req, res) => {
  db.all("SELECT * FROM tasks WHERE userId=?", [req.user.id], (err, tasks) => {
    res
      .status(200)
      .json({ message: "Get Details of Task Successfully!", tasks });
  });
});
...
app.put("/updatetasks/:id", authenticateJWT, (req, res) => {
  ...
  db.run(
    "UPDATE tasks SET title=?,status=? WHERE id=? AND userId=?",
...
app.delete("/deleteprofile", authenticateJWT, (req, res) => {
  db.run("DELETE FROM users WHERE id=?", [req.user.id], (err) => {
    if (err)
      return res.status(400).send({ message: "Profile Deletion Failed!" });
